...
export default function ExportProgressButton({ rubricProgress, studentName = 'estudiante', documentId = 'documento' }) {
  ...
  const exportJSON = () => {
    ...
    Object.entries(rubricProgress).forEach(([rubricId, data]) => {
      if (data?.scores && data.scores.length > 0) {
        ...
        exportData.artefactos[rubricId] = {
          nombre: ARTEFACTO_NAMES[rubricId],
    ...
    downloadFile(json, `progreso_${documentId}_${new Date().toISOString().split('T')[0]}.json`, 'application/json');
  };
