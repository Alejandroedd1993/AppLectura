...
try {
    ...
    Write-Host "FORTALEZAS ($($data.fortalezas.Count))" -ForegroundColor Green
    foreach ($fortaleza in $data.fortalezas) {
        Write-Host "   + $fortaleza" -ForegroundColor Green
    }
    Write-Host ""
    
    Write-Host "AREAS DE MEJORA ($($data.mejoras.Count))" -ForegroundColor Yellow
    ...
    Write-Host "SIGUIENTES PASOS ($($data.siguientesPasos.Count))" -ForegroundColor Magenta
    $pasoNum = 1
